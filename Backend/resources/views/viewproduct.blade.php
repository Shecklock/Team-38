...
<div class="py-3 py-md-5 bg-light">
        ...
        <div class="row">
            ...
            <div class="product-view-container">
                ...
                <div class="product-details">
					...
					@php
					$quantity = $product->sizes->first()->pivot->quantity; // Again, 'quantity' should be lowercase
					@endphp


   					 @if($quantity > 10)
        				<span id="stockLevel" class="label-stock bg-success">In Stock</span>
   					 @elseif($quantity > 0)
       					 <span id="stockLevel" class="label-stock bg-warning">Low in Stock</span>
    				 @else
    ...
    <form action="{{ route('add-to-basket', ['productId' => $product->ProductID]) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select name="size" id="size" onchange="updateStockLevel(this.value)" class="form-select">
                @foreach ($product->sizes as $size)
                    <option value="{{ $size->id }}">{{ $size->size }} ({{ $size->pivot->quantity }} in stock)</option>
                @endforeach
            </select>
        </div>
            			...
            			<script>
            				function updateStockLevel(sizeID) {
            					// var productID = <?php echo $product->id; ?>;
            					// var stockQuantity = [productID, " ", sizeID];
            					// document.getElementById("stockLevel").innerHTML = productID;
            				}
						</script>
                    </div>
                </div>
